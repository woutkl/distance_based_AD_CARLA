...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes,
    ...
    int benchmark, int benchmark_layers, char *framefiles_name, int fps_step)
{
    ...
    // Open the file with all the image names, the image names include the frame numbers
    FILE* waitframes_file = fopen(framefiles_name, "r");
    char *waitframes[256];
    char* input_copy[256];
    float waituntil;
    float now;
    char* ptr;
    fprintf(stderr, "Fps parameter: %d\n", fps_step);
    // Get the number of frames per second used for the frame numbers
    int fps_simulator = fps_step;
    while(1){
        ++count;
        {
            ...
            fgets(waitframes, sizeof(waitframes), waitframes_file);
            strtok(waitframes, "\n");
            fprintf(stderr, "%s\n", waitframes);
            // Get the timestamp to wait until proceeding to detect an object
            waituntil = ((double) strtol(waitframes, &ptr, 10))*1000000/fps_simulator;
            // Get the current timestamp
            now = (get_time_point()-start_time_lim);
            fprintf(stderr, "Wait until: %f\n", waituntil);
            fprintf(stderr, "Time to wait: %f\n", waituntil-now);
            // Check if the time to wait is positive
            if (waituntil-now > 0){
                // If the time to wait is positive, calculate and wait for the appropriate amount of time
                usleep((int) round(waituntil-now));
            }
